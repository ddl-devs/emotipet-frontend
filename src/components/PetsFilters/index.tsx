    ...
    <>
      <div className="flex flex-col gap-4">
        <div className="flex -mt-2 flex-row gap-4 justify-start min-w-[257px] items-end flex-wrap">
          <ProfileInputFilter wid="100px" input={name} label="Nome" id="name" placeholder="Nome" onChange={(e) => setName(e.target.value)} />
          <ProfileInputFilter wid="100px" input={breed} label="Raça:" id="breed" placeholder="Raça" onChange={(e) => setBreed(e.target.value)} />
          <ProfileInputFilter options={{ MALE: "Macho", FEMALE: "Fêmea" }} select={true} wid="100px" input={gender} label="Gênero:" id="gender" placeholder="Gênero" onChange={(e) => setGender(e.target.value)} />
          <ProfileInputFilter options={{ DOG: "Cachorro", CAT: "Gato" }} select={true} wid="100px" input={species} label="Espécie:" id="species" placeholder="Espécie" onChange={(e) => setSpecies(e.target.value)} />
          ...
          <button className='gap-1 flex justify-center items-center bg-blue rounded-full text-white text-lg font-medium h-10 w-24'>
            Adicionar
          </button>
        </div>
      </div>
      <div className='flex justify-start items-center flex-col w-full'>
        ...
        {pets.length === 0 ? (
          ...
          >
            <CardPet name={pet.name} photoUrl={pet.photoUrl} id={pet.id}/>
          </div>
            ))}
          </div>
        )}
